...
struct ContentView: View {
	// MARK:-- Order injetado externamente, evitando recriação a cada exibição
		...
		NavigationStack {
			ScrollView {
				VStack(spacing: 20) {
					...
					GroupBox("Selecione os Produtos") {
						productSelectionSection()
					}
					
					GroupBox("Valor do Pedido") {
						orderSummarySection()
					}
				}
				.padding()
				
				// Botão para navegação para a tela de Checkout.
